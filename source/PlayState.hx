...
class PlayState extends MusicBeatState
{
	...
	var healthDrainAmount:Float = 0.08;
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						{
							case 2:
								if (!PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && FlxG.save.data.cameraMovement)
								{
	...
	*/
	...
	override function beatHit()
	{
		...
		if (!dad.animation.curAnim.name.startsWith("sing") && SONG.player2 == 'gf')
			{
				dad.dance();
				if(!PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
				{
